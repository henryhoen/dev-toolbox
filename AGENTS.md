...
`update-dev-toolbox.sh` may update the **manager programs themselves**, but must not automatically advance those managed SDK/toolchain versions.
...
- Setup/update scripts should use `set -Eeuo pipefail` unless a deliberate reason requires otherwise.
- Quote variables.
